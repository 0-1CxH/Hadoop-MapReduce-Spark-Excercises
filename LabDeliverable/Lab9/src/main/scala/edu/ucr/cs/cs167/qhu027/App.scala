  ...
  def main(args : Array[String]) {
    ...
    try {
      if (method.equals("regression"))
      {
        ...
        val indexer = new StringIndexer().setInputCol("sentiment").setOutputCol("label").setHandleInvalid("skip")
        val mlr = new LogisticRegression().setFeaturesCol("features").setLabelCol("label")
        val pipeline = new Pipeline()
          .setStages(Array(tokenizer, hashingTF, indexer, mlr))
        val paramGrid = new ParamGridBuilder()
          .addGrid(hashingTF.numFeatures, Array(10, 100, 1000))
          .addGrid(mlr.regParam, Array(0.01, 0.1, 0.3, 0.8))
          .build()
